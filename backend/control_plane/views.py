...
<body>
    ...
    <div class="download-item primary-download">
        <a href="/api/download/pool-node/" class="btn btn-primary">下载 算力池节点版 (v3.25 推荐)</a>
        <span class="tag tag-recommended">推荐</span>
        <div class="desc">自动修正屏幕源 ID，彻底解决 source not found 问题 (推荐)</div>
    </div>
...
class DownloadPoolNodeView(APIView):
    ...
    def get(self, request):
        file_path = '/SuanLiJia/SLC-Pool-Node-GUI-v3.24.1.zip'
        if not os.path.exists(file_path):
            return HttpResponse('File not found', status=404)
        
        response = FileResponse(
            open('/SuanLiJia/SLC-Pool-Node-GUI-v3.25.zip', 'rb'),
            content_type='application/zip'
        )
        response['Content-Disposition'] = 'attachment; filename="SLC-Pool-Node-GUI-v3.25.zip"'
        return response
