...
class LeasesPage extends StatelessWidget {
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Text('Lease ID: ${lease.leaseId}', style: const TextStyle(fontWeight: FontWeight.bold)),
                      ...
                      if (lease.startedAt != null)
                        Text('开始时�? ${lease.startedAt!.toLocal()}'),
                      if (lease.endedAt != null)
                        Text('结束时间: ${lease.endedAt!.toLocal()}'),
                      if (lease.endReason != null)
                         Text('结束原因: ${lease.endReason!.name}', style: const TextStyle(color: Colors.red)),
                      
                      const SizedBox(height: 16),
                      Row(
                        mainAxisAlignment: MainAxisAlignment.end,
                        children: [
                          if (lease.status == LeaseStatus.ready || lease.status == LeaseStatus.active) ...[
                            ElevatedButton(
                              onPressed: () {
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (_) => LeaseRemoteControlPage(lease: lease),
                                  ),
                                );
                              },
                            ...
                            OutlinedButton(
                              onPressed: () => _confirmRelease(context, lease),
  ...
  void _confirmRelease(BuildContext context, Lease lease) {
    showDialog(
      context: context,
              ...
              try {
                ...
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('释放指令已发�?)),
                  );
                }
              } catch (e) {
                if (context.mounted) {
