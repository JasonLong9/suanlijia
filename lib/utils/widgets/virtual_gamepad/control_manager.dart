...
class ControlManager {
  ...
  void createWASDJoystick({
    ...
    int? sneakKeyCode,
    int? sprintKeyCode,
    double sneakThreshold = 0.3,
    double sprintThreshold = 0.8,
    bool enableArcadeMode = false,
    double arcadeThreshold = 0.5,
    double centerX = 0.2,
    double centerY = 0.8,
    double size = 0.1,
  ...
  void updateControl(
    ...
    int? sneakKeyCode,
    int? sprintKeyCode,
    double? sneakThreshold,
    double? sprintThreshold,
    bool? enableArcadeMode,
    double? arcadeThreshold,
  }) {
    ...
    if (index != -1) {
      ...
      if (control is JoystickControl) {
        ...
        _controls[index] = ButtonControl(
          ...
          isToggleMode: isToggleMode ?? control.isToggleMode,
        );
        ...
        _controls[index] = WASDJoystickControl(
          ...
          sneakKeyCode: sneakKeyCode ?? control.sneakKeyCode,
          sprintKeyCode: sprintKeyCode ?? control.sprintKeyCode,
          sneakThreshold: sneakThreshold ?? control.sneakThreshold,
          sprintThreshold: sprintThreshold ?? control.sprintThreshold,
          enableArcadeMode: enableArcadeMode ?? control.enableArcadeMode,
          arcadeThreshold: arcadeThreshold ?? control.arcadeThreshold,
          color: color ?? control.color,
          opacity: opacity ?? control.opacity,
        );
      }
      ...
      for (final control in newControls) {
        ...
        if (control is JoystickControl) {
          ...
          _controls.add(ButtonControl(
            ...
            isToggleMode: control.isToggleMode,
          ));
        } else if (control.type == 'mouseModeButton') {
          ...
          _controls.add(WASDJoystickControl(
            ...
            sneakKeyCode: control.sneakKeyCode,
            sprintKeyCode: control.sprintKeyCode,
            sneakThreshold: control.sneakThreshold,
            sprintThreshold: control.sprintThreshold,
            enableArcadeMode: control.enableArcadeMode,
            arcadeThreshold: control.arcadeThreshold,
            color: control.color,
            opacity: control.opacity,
          ));
        }
